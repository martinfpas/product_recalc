...
class product_product(osv.Model):
    ...
    def _get_start_stock_and_cost(self, cr, uid, ids, dates, digits, context):
        ...
        _sql_string = '  SELECT sm.id \n' \
                      ...
                      ' ORDER BY sm.date;'.format(product=ids[0]).replace(',)', ')')
        ...
        return [0, round(_cost, digits['price']), False]

    def _get_last_purchase_amount_and_cost(self, cr, uid, ids, digits, context):
        _stock = 0.0
        _cost = 0.0
        ...
        if True:
            ...
            _sql_string = 'SELECT id \n' \
                          ...
                          '            OR location_dest_id IN {locations})'.format(product = ids[0], locations = tuple(_location_ids)).replace(',)', ')')
            cr.execute(_sql_string)
